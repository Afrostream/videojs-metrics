...
(function (window, videojs) {
  ...
  metrics = function (options) {
    var settings = videojs.util.mergeOptions(defaults, options),
      player = this, setupTriggers, eventHandler, notify, xhr, browserInfo = getBrowser(),
    ...
    setupTriggers = function () {
      for (var i = settings.trackEvents.length - 1; i >= 0; i--) {
        player.on(settings.trackEvents[i], eventHandler.bind(this));
      }
    };
    ...
    notify = function (evt) {
      ...
      evt.os_version = browserInfo.version;
      ...
      evt.html5_video = player.techName;
